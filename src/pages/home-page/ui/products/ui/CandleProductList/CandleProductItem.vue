	...
	<li class="candle-product-item" @click="onClick">
		...
		<div class="candle-product-item__main">
			<p class="candle-product-item__name">
				<span
					>{{ props.product.title }}<span v-if="shouldHaveComma">,</span>
				</span>
				<span v-if="props.product.amount" class="candle-product-item__amount">
					{{ props.product.amount }} {{ props.product.unit }}
				</span>
			</p>
			<div class="candle-product-item__price">
				<button @click="onClick">
					<p class="candle-product-item__price-more underline bold black">
						{{ $t('more') }}
						<svg
							class="candle-product-item__plus-icon"
							width="12"
							height="13"
							viewBox="0 0 12 13"
							fill="none"
